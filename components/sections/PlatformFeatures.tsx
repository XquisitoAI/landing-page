...
const PlatformFeatures: React.FC<PlatformFeaturesProps> = ({
  ...
  const [shouldPlayTapOrderVideo, setShouldPlayTapOrderVideo] = useState(false)
  const [hasReachedTapOrderPanel, setHasReachedTapOrderPanel] = useState(false)
  const [shouldPlayTapPayVideo, setShouldPlayTapPayVideo] = useState(false)
  const [hasReachedTapPayPanel, setHasReachedTapPayPanel] = useState(false)
  const [shouldPlayFlexBillVideo, setShouldPlayFlexBillVideo] = useState(false)
  const [hasReachedFlexBillPanel, setHasReachedFlexBillPanel] = useState(false)
  ...
  const handleScroll = useCallback(() => {
    ...
    // Check if the section is "stuck" (sticky positioned at top)
    const isSticky = rect.top <= 0 && rect.bottom > viewportHeight
    const isAtTapOrderPanel = clampedProgress >= 0 && clampedProgress <= 0.21
    const isAtTapPayPanel = clampedProgress > 0.21 && clampedProgress <= 0.42
    const isAtFlexBillPanel = clampedProgress > 0.42 && clampedProgress <= 0.63
    
    // Start countdown only when section becomes sticky AND we pass the first threshold
    if (isSticky && clampedProgress > 0 && !hasReachedTapOrderPanel) {
      setHasReachedTapOrderPanel(true)
      setTimeout(() => {
        setShouldPlayTapOrderVideo(true)
      }, 3000) // 3 second delay after passing 0%
    }
    
    // Start countdown for Tap & Pay panel after passing 21%
    if (clampedProgress > 0.21 && !hasReachedTapPayPanel) {
      setHasReachedTapPayPanel(true)
      setTimeout(() => {
        setShouldPlayTapPayVideo(true)
      }, 3000) // 3 second delay after passing 21%
    }
    
    // Start countdown for Flex Bill panel after passing 42%
    if (clampedProgress > 0.42 && !hasReachedFlexBillPanel) {
      setHasReachedFlexBillPanel(true)
      setTimeout(() => {
        setShouldPlayFlexBillVideo(true)
      }, 3000) // 3 second delay after passing 42%
    }
    
    // Reset Tap Order & Pay when leaving its area
    if (clampedProgress > 0.21) {
      if (hasReachedTapOrderPanel) {
        setHasReachedTapOrderPanel(false)
        setShouldPlayTapOrderVideo(false)
      }
    }
    
    // Reset Tap & Pay when leaving its area  
    if (clampedProgress <= 0.21 || clampedProgress > 0.42) {
      if (hasReachedTapPayPanel) {
        setHasReachedTapPayPanel(false)
        setShouldPlayTapPayVideo(false)
      }
    }
    
    // Reset Flex Bill when leaving its area
    if (clampedProgress <= 0.42 || clampedProgress > 0.63) {
      if (hasReachedFlexBillPanel) {
        setHasReachedFlexBillPanel(false)
        setShouldPlayFlexBillVideo(false)
      }
    }
  }, [shouldPlayTapOrderVideo, hasReachedTapOrderPanel, shouldPlayTapPayVideo, hasReachedTapPayPanel, shouldPlayFlexBillVideo, hasReachedFlexBillPanel])
                    ...
                    {panel.imageUrl && (
                      <div className="w-full md:w-1/2 flex justify-center items-center mt-6 md:mt-0">
                        {index === 0 ? (
                          ...
                          >
                            <video 
                              key={shouldPlayTapOrderVideo ? 'playing' : 'paused'}
                              autoPlay={shouldPlayTapOrderVideo} 
                              loop 
                              muted 
                              playsInline
                          ...
                          >
                            <video 
                              key={shouldPlayTapPayVideo ? 'playing' : 'paused'}
                              autoPlay={shouldPlayTapPayVideo} 
                              loop 
                              muted 
                              playsInline
                              className="w-full h-full object-cover"
                          ...
                          >
                            <video 
                              key={shouldPlayFlexBillVideo ? 'playing' : 'paused'}
                              autoPlay={shouldPlayFlexBillVideo} 
                              loop 
                              muted 
                              playsInline
                              className="w-full h-full object-cover"
